...
まずSSH鍵を作成します．
アプリケーションメニューから`Passwords and Keys`を起動し，左上の`+`ボタンから`Secure Shell key`を選択します．
出てきたダイアログで`Desctiption`に適当な識別子（`<ユーザ名>@<ホスト名>` など）を入力し，`Generate`をクリックします．
...
`Add`をクリックし，出現するダイアログに先程メモした公開鍵をコピー＆ペーストしてください．
